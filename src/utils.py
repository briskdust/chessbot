...
def evaluate_king_safety_middlegame(board):
    ...
    for color in [chess.WHITE, chess.BLACK]:
        ...
        if is_castled:
            ...
            pawn_shield_score -= (
                expected_shield_pawns
                - min(pawn_shield_score // 10, expected_shield_pawns)
            ) * 15
...
def evaluate_piece_coordination(board, is_endgame):
    ...
    for color in [chess.WHITE, chess.BLACK]:
        ...
        if not is_endgame:
            initial_knight_squares = (
                [chess.B1, chess.G1] if color == chess.WHITE else [chess.B8, chess.G8]
            )
            initial_bishop_squares = (
...
def evaluate_rook_placement(board):
    ...
    for color in [chess.WHITE, chess.BLACK]:
        ...
        for rook_square in rooks:
            ...
            if (color == chess.WHITE and rook_rank == 6) or (
                color == chess.BLACK and rook_rank == 1
            ):
                # Check if enemy king is on the back rank
                enemy_king_rank = (
                    chess.square_rank(board.king(not color))
                    if board.king(not color)
                    else -1
                )
                if (color == chess.WHITE and enemy_king_rank == 7) or (
                    color == chess.BLACK and enemy_king_rank == 0
                ):
...
def evaluate_bishop_pair(board):
    ...
    if len(white_bishops) >= 2:
        white_colors = set(
            chess.square_file(sq) % 2 == chess.square_rank(sq) % 2
    ...
    for color in [chess.WHITE, chess.BLACK]:
        ...
        for knight_square in knights:
            ...
            if potential_outpost:
                ...
                pawn_defenders = [
                    s
                    for s in board.attackers(color, knight_square)
...
def find_best_move(
    ...
    best_move = legal_moves[
        0
    ]  # Default to first legal move to ensure we always have a move
